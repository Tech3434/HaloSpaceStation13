...
/obj/item/organ/internal/liver/process()
	...
	if(owner.life_tick % PROCESS_ACCURACY == 0)
		...
		var/filter_bay_effect = 3
		if(is_bruised())
			filter_bay_effect -= 1
		if(is_broken())
			filter_bay_effect -= 2
		// Robotic organs filter_bay better but don't get benefits from dylovene for filter_baying.
		...
		// If you're not filter_baying well, you're going to take damage. Even more if you have alcohol in you.
		if(filter_bay_effect < 2)
			owner.adjustToxLoss(PROCESS_ACCURACY * 0.5 * max((3 - filter_bay_effect), 0) * (1 + owner.chem_effects[CE_ALCOHOL_TOXIC] + (owner.chem_effects[CE_ALCOHOL] / 2)))
