...
/spell/targeted/choose_targets(mob/user = usr)
	...
	if(compatible_mobs && compatible_mobs.len)
		for(var/mob/living/target in targets) //filter_bays out all the non-compatible mobs
...
/spell/targeted/cast(var/list/targets, mob/user)
	for(var/mob/living/target in targets)
		if(range >= 0)
			if(!(target in view_or_range(range, holder, selection_type))) //filter_bay at time of casting
				targets -= target
				continue
