...
proc/Stagger(mob/M,d) //Technically not a filter_bay, but it relates to drunkenness.
...
proc/Ellipsis(original_msg, chance = 50)
	...
	return new_msg
/*
RadioChat filter_bay.
args:
message - returns a distorted version of this
distortion_chance - the chance of a filter_bay being applied to each character.
