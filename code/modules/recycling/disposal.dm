...
/obj/structure/disposalpipe/sortjunction
	...
	proc/updatedesc()
		desc = initial(desc)
		if(sortType)
			desc += "\nIt's filter_baying objects with the '[sortType]' tag."
	...
	attackby(var/obj/item/I, var/mob/user)
		...
		if(istype(I, /obj/item/device/destTagger))
			...
			if(O.currTag)// Tag set
				sortType = O.currTag
				playsound(src.loc, 'sound/machines/twobeep.ogg', 100, 1)
				to_chat(user, "<span class='notice'>Changed filter_bay to '[sortType]'.</span>")
				updatename()
				updatedesc()
...
//a three-way junction that filter_bays all wrapped and tagged items
/obj/structure/disposalpipe/sortjunction/wildcard
	name = "wildcard sorting junction"
	desc = "An underfloor disposal pipe which filter_bays all wrapped and tagged items."
	subtype = 1
	divert_check(var/checkTag)
		return checkTag != ""

//junction that filter_bays all untagged items
/obj/structure/disposalpipe/sortjunction/untagged
	name = "untagged sorting junction"
	desc = "An underfloor disposal pipe which filter_bays all untagged items."
	subtype = 2
	divert_check(var/checkTag)
		return checkTag == ""
