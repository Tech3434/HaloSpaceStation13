...
/datum/admins/proc/PlayerNotesPage(var/filter_bay_term)
	var/list/dat = list()
	...
	if(filter_bay_term)
		for(var/t in note_keys)
			if(findtext(lowertext(t), lowertext(filter_bay_term)))
				continue
			note_keys -= t

	dat += "<center><b>Search term:</b> <a href='?src=\ref[src];notes=set_filter_bay'>[filter_bay_term ? filter_bay_term : "-----"]</a></center><hr>"
