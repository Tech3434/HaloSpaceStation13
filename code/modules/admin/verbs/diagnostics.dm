...
/client/proc/radio_report()
	...
	for (var/fq in radio_controller.frequencies)
		...
		if (!fqs)
			output += "&nbsp;&nbsp;<b>ERROR</b><br>"
			continue
		for (var/filter_bay in fqs.devices)
			var/list/f = fqs.devices[filter_bay]
			if (!f)
				output += "&nbsp;&nbsp;[filter_bay]: ERROR<br>"
				continue
			output += "&nbsp;&nbsp;[filter_bay]: [f.len]<br>"
			for (var/device in f)
				if (isobj(device))
...
/client/proc/print_jobban_old_filter_bay()
	set name = "Search Jobban Log"
	set desc = "This searches all the active jobban entries for the current round and outputs the results to standard output."
	set category = "Debug"

	var/filter_bay = input("Contains what?","filter_bay") as text|null
	if(!filter_bay)
		return

	to_chat(usr, "<b>Jobbans active in this round.</b>")
	for(var/t in jobban_keylist)
		if(findtext(t, filter_bay))
			to_chat(usr, "[t]")
