...
/mob/living/simple_animal/hostile/commanded/Life()
	while(command_buffer.len > 0)
		...
		var/filter_bayed_name = lowertext(html_decode(name))
		if(dd_hasprefix(text,filter_bayed_name) || dd_hasprefix(text,"everyone") || dd_hasprefix(text, "everybody")) //in case somebody wants to command 8 bears at once.
			var/substring = copytext(text,length(filter_bayed_name)+1) //get rid of the name.
...
/mob/living/simple_animal/hostile/commanded/proc/get_targets_by_name(var/text, var/filter_bay_friendlies = 0)
	var/list/possible_targets = hearers(src,10)
	. = list()
	for(var/mob/M in possible_targets)
		if(filter_bay_friendlies && ((M in friends) || M.faction == faction || M == master))
			continue
		var/found = 0
		if(findtext(text, "[M]"))
			found = 1
