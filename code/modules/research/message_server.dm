...
/obj/machinery/message_server
	...
	//Spam filter_baying stuff
	var/list/spamfilter_bay = list("You have won", "your prize", "male enhancement", "shitcurity", \
			"are happy to inform you", "account number", "enter your PIN")
			//Messages having theese tokens will be rejected by server. Case sensitive
	var/spamfilter_bay_limit = MESSAGE_SERVER_DEFAULT_SPAM_LIMIT	//Maximal amount of tokens
...
/obj/machinery/message_server/proc/send_pda_message(var/recipient = "",var/sender = "",var/message = "")
	var/result
	for (var/token in spamfilter_bay)
...
/obj/machinery/message_server/attackby(obj/item/weapon/O as obj, mob/living/user as mob)
	if (active && !(stat & (BROKEN|NOPOWER)) && (spamfilter_bay_limit < MESSAGE_SERVER_DEFAULT_SPAM_LIMIT*2) && \
		istype(O,/obj/item/weapon/circuitboard/message_monitor))
		spamfilter_bay_limit += round(MESSAGE_SERVER_DEFAULT_SPAM_LIMIT / 2)
		user.drop_item()
		qdel(O)
		to_chat(user, "You install additional memory and processors into message server. Its filter_baying capabilities been enhanced.")
	else
		..(O, user)
