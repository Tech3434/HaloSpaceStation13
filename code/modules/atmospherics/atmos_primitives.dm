/*
	...
	These procs generalize various processes used by atmos machinery, such as pumping, filter_baying, or scrubbing gas, allowing them to be reused elsewhere.
	If no gas was moved/pumped/filter_bayed/whatever, they return a negative number.
...
//Selectively moves specified gasses one gas_mixture to another and returns the amount of power needed (assuming 1 second), or -1 if no gas was filter_bayed.
//filter_baying - A list of gasids to be scrubbed from source
...
/proc/scrub_gas(var/obj/machinery/M, var/list/filter_baying, var/datum/gas_mixture/source, var/datum/gas_mixture/sink, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1

	filter_baying = filter_baying & source.gas	//only filter_bay gasses that are actually there. DO NOT USE &=

	//Determine the specific power of each filter_bayable gas type, and the total amount of filter_bayable gas (gasses selected to be scrubbed)
	var/total_filter_bayable_moles = 0			//the total amount of filter_bayable gas
	var/list/specific_power_gas = list()	//the power required to remove one mole of pure gas, for each gas type
	for (var/g in filter_baying)
		if (source.gas[g] < MINIMUM_MOLES_TO_filter_bay)
			continue

		var/specific_power = calculate_specific_power_gas(g, source, sink)/ATMOS_filter_bay_EFFICIENCY
		specific_power_gas[g] = specific_power
		total_filter_bayable_moles += source.gas[g]

	if (total_filter_bayable_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1

	//now that we know the total amount of filter_bayable gas, we can calculate the amount of power needed to scrub one mole of gas
	var/total_specific_power = 0		//the power required to remove one mole of filter_bayable gas
	for (var/g in filter_baying)
		var/ratio = source.gas[g]/total_filter_bayable_moles //this converts the specific power per mole of pure gas to specific power per mole of scrubbed gas
		total_specific_power += specific_power_gas[g]*ratio

	//Figure out how much of each gas to filter_bay
	if (isnull(total_transfer_moles))
		total_transfer_moles = total_filter_bayable_moles
	else
		total_transfer_moles = min(total_transfer_moles, total_filter_bayable_moles)
	...
	if (total_transfer_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	for (var/g in filter_baying)
		...
		//filter_bay gas in proportion to the mole ratio
		transfer_moles = min(transfer_moles, total_transfer_moles*(source.gas[g]/total_filter_bayable_moles))

		//use update=0. All the filter_bayed gasses are supposed to be added simultaneously, so we update after the for loop.
...
//Generalized gas filter_baying proc.
//filter_baying is a bit different from scrubbing. Instead of selectively moving the targeted gas types from one gas mix to another, filter_baying splits
...
//filter_baying - A list of gasids to be filter_bayed. These gasses get moved to sink_filter_bayed, while the other gasses get moved to sink_clean.
//total_transfer_moles - Limits the amount of moles to input. The actual amount of gas filter_bayed may also be limited by available_power, if given.
//available_power - the maximum amount of power that may be used when filter_baying gas. If null then the filter_baying is not limited by power.
/proc/filter_bay_gas(var/obj/machinery/M, var/list/filter_baying, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_filter_bayed, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	var/total_filter_bayable_moles = 0		//the total amount of filter_bayable gas
	var/total_unfilter_bayable_moles = 0	//the total amount of non-filter_bayable gas
	var/list/specific_power_gas = list()	//the power required to remove one mole of pure gas, for each gas type
	for (var/g in source.gas)
		if (source.gas[g] < MINIMUM_MOLES_TO_filter_bay)
			continue

		if (g in filter_baying)
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_filter_bayed)/ATMOS_filter_bay_EFFICIENCY
			total_filter_bayable_moles += source.gas[g]
		else
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_clean)/ATMOS_filter_bay_EFFICIENCY
			total_unfilter_bayable_moles += source.gas[g]
	...
	var/filter_bayed_power_used = 0		//power used to move filter_bayable gas to sink_filter_bayed
	var/unfilter_bayed_power_used = 0	//power used to move unfilter_bayable gas to sink_clean
	for (var/g in removed.gas)
		var/power_used = specific_power_gas[g]*removed.gas[g]

		if (g in filter_baying)
			//use update=0. All the filter_bayed gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filter_bayed.adjust_gas_temp(g, removed.gas[g], removed.temperature, update=0)
			removed.adjust_gas(g, -removed.gas[g], update=0)
			filter_bayed_power_used += power_used
		else
			unfilter_bayed_power_used += power_used

	sink_filter_bayed.update_values()
	removed.update_values()

	sink_clean.merge(removed)

	return filter_bayed_power_used + unfilter_bayed_power_used

//For omni devices. Instead filter_baying is an associative list mapping gasids to gas mixtures.
//I don't like the copypasta, but I decided to keep both versions of gas filter_baying as filter_bay_gas is slightly faster (doesn't create as many temporary lists, doesn't call update_values() as much)
//filter_bay_gas can be removed and replaced with this proc if need be.
/proc/filter_bay_gas_multi(var/obj/machinery/M, var/list/filter_baying, var/datum/gas_mixture/source, var/datum/gas_mixture/sink_clean, var/total_transfer_moles = null, var/available_power = null)
	if (source.total_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1
	...
	for (var/g in source.gas)
		...
		if (g in filter_baying)
			var/datum/gas_mixture/sink_filter_bayed = filter_baying[g]
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_filter_bayed)/ATMOS_filter_bay_EFFICIENCY
			total_filter_bayable_moles += source.gas[g]
		else
			specific_power_gas[g] = calculate_specific_power_gas(g, source, sink_clean)/ATMOS_filter_bay_EFFICIENCY
	...
	var/list/filter_bayed_power_used = list()		//power used to move filter_bayable gas to the filter_bayed gas mixes
	var/unfilter_bayed_power_used = 0	//power used to move unfilter_bayable gas to sink_clean
	for (var/g in removed.gas)
		var/power_used = specific_power_gas[g]*removed.gas[g]
		...
		if (g in filter_baying)
			var/datum/gas_mixture/sink_filter_bayed = filter_baying[g]
			//use update=0. All the filter_bayed gasses are supposed to be added simultaneously, so we update after the for loop.
			sink_filter_bayed.adjust_gas_temp(g, removed.gas[g], removed.temperature, update=1)
			removed.adjust_gas(g, -removed.gas[g], update=0)
			if (power_used)
				filter_bayed_power_used[sink_filter_bayed] = power_used
		else
			unfilter_bayed_power_used += power_used

	removed.update_values()

	var/power_draw = unfilter_bayed_power_used
	for (var/datum/gas_mixture/sink_filter_bayed in filter_bayed_power_used)
		power_draw += filter_bayed_power_used[sink_filter_bayed]
...
/proc/mix_gas(var/obj/machinery/M, var/list/mix_sources, var/datum/gas_mixture/sink, var/total_transfer_moles = null, var/available_power = null)
	...
	for (var/datum/gas_mixture/source in mix_sources)
		if (source.total_moles < MINIMUM_MOLES_TO_filter_bay)
		...
		source_specific_power[source] = calculate_specific_power(source, sink)*mix_ratio/ATMOS_filter_bay_EFFICIENCY
		total_specific_power += source_specific_power[source]
		total_input_volume += source.volume
		total_input_moles += source.total_moles

	if (total_mixing_moles < MINIMUM_MOLES_TO_filter_bay) //if we cant transfer enough gas just stop to avoid further processing
		return -1
