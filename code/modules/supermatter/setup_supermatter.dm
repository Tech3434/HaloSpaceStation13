...
/datum/admins/proc/setup_supermatter()
	...
	for(var/obj/effect/engine_setup/filter_bay/F in world)
		F.coolant = response
...
// Sets up filter_bays. This assumes filter_bays are set to filter_bay out N2 back to the core loop by default!
/obj/effect/engine_setup/filter_bay/
	name = "Omni filter_bay Marker"
	var/coolant = null

/obj/effect/engine_setup/filter_bay/activate()
	..()
	var/obj/machinery/atmospherics/omni/filter_bay/F = locate() in get_turf(src)
	if(!F)
		log_and_message_admins("## WARNING: Unable to locate omni filter_bay at [x] [y] [z]!")
		return SETUP_WARNING
	if(!coolant)
		log_and_message_admins("## WARNING: No coolant type set at [x] [y] [z]!")
		return SETUP_WARNING

	// Non-nitrogen coolant, adjust the filter_bay's config first.
	if(coolant != "N2")
		for(var/datum/omni_port/P in F.ports)
			...
			if(coolant == "PH")
				...
				log_and_message_admins("## WARNING: Inapropriate filter_bay coolant type set at [x] [y] [z]!")
				return SETUP_WARNING
		F.rebuild_filter_baying_list()
