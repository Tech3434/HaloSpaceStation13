...
/datum/unit_test/atmos_machinery/conserve_moles/scrub_gas/start_test()
	var/list/filter_baying = gas_data.gases
	...
	for(var/case_name in test_cases)
		...
		scrub_gas(null, filter_baying, after_gas_mixes["source"], after_gas_mixes["sink"], null, INFINITY)

		check_moles_conserved(case_name, before_gas_mixes, after_gas_mixes)

	return 1

/datum/unit_test/atmos_machinery/conserve_moles/filter_bay_gas
	name = "ATMOS MACHINERY: filter_bay_gas() Conserves Moles"

/datum/unit_test/atmos_machinery/conserve_moles/filter_bay_gas/start_test()
	var/list/filter_baying = gas_data.gases
	...
	for(var/case_name in test_cases)
		...
		filter_bay_gas(null, filter_baying, after_gas_mixes["source"], after_gas_mixes["sink"], after_gas_mixes["source"], null, INFINITY)

		check_moles_conserved(case_name, before_gas_mixes, after_gas_mixes)

	return 1

/datum/unit_test/atmos_machinery/conserve_moles/filter_bay_gas_multi
	name = "ATMOS MACHINERY: filter_bay_gas_multi() Conserves Moles"

/datum/unit_test/atmos_machinery/conserve_moles/filter_bay_gas_multi/start_test()
	for(var/case_name in test_cases)
		var/gas_mix_data = test_cases[case_name]
		var/list/before_gas_mixes = create_gas_mixes(gas_mix_data)
		var/list/after_gas_mixes = create_gas_mixes(gas_mix_data)

		var/list/filter_baying = list()
		for(var/gasid in gas_data.gases)
			filter_baying[gasid] = after_gas_mixes["sink"] //just filter_bay everything to sink

		filter_bay_gas_multi(null, filter_baying, after_gas_mixes["source"], after_gas_mixes["sink"], null, INFINITY)
