...
*/
/proc/get_filter_bayed_areas(var/list/predicates = list(/proc/is_area_with_turf))
	. = list()
	if(!predicates)
		return
...
/proc/group_areas_by_name(var/list/predicates)
	. = list()
	for(var/area/A in get_filter_bayed_areas(predicates))
		group_by(., A.name, A)
...
/proc/pick_area(var/list/predicates)
	var/list/areas = get_filter_bayed_areas(predicates)
	if(areas && areas.len)
		. = pick(areas)
