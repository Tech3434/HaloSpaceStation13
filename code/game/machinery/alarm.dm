	...
	switch(screen)
		...
		if(AALARM_SCREEN_SCRUB)
			...
			for(var/id_tag in alarm_area.air_scrub_names)
				...
				scrubbers[scrubbers.len]["filter_bays"] += list(list("name" = "Oxygen",			"command" = "o2_scrub",	"val" = info["filter_bay_o2"]))
				scrubbers[scrubbers.len]["filter_bays"] += list(list("name" = "Nitrogen",		"command" = "n2_scrub",	"val" = info["filter_bay_n2"]))
				scrubbers[scrubbers.len]["filter_bays"] += list(list("name" = "Carbon Dioxide", "command" = "co2_scrub","val" = info["filter_bay_co2"]))
				scrubbers[scrubbers.len]["filter_bays"] += list(list("name" = "Toxin"	, 		"command" = "tox_scrub","val" = info["filter_bay_phoron"]))
				scrubbers[scrubbers.len]["filter_bays"] += list(list("name" = "Nitrous Oxide",	"command" = "n2o_scrub","val" = info["filter_bay_n2o"]))
			data["scrubbers"] = scrubbers
		if(AALARM_SCREEN_MODE)
			var/modes[0]
			modes[++modes.len] = list("name" = "filter_baying - Scrubs out contaminants", 			"mode" = AALARM_MODE_SCRUBBING,		"selected" = mode == AALARM_MODE_SCRUBBING, 	"danger" = 0)
