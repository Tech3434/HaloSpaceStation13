...
/obj/item/pipe/New(var/loc, var/pipe_type as num, var/dir as num, var/obj/machinery/atmospherics/make_from = null)
	...
	if (make_from)
		...
		else if(istype(make_from, /obj/machinery/atmospherics/trinary/filter_bay/m_filter_bay))
			src.pipe_type = PIPE_GAS_filter_bay_M
		...
		else if(istype(make_from, /obj/machinery/atmospherics/trinary/filter_bay))
			src.pipe_type = PIPE_GAS_filter_bay
		...
		else if(istype(make_from, /obj/machinery/atmospherics/omni/filter_bay))
			src.pipe_type = PIPE_OMNI_filter_bay
///// Z-Level stuff
...
/obj/item/pipe/proc/get_pipe_dir()
	...
	switch(pipe_type)
		...
		if(PIPE_CONNECTOR,PIPE_UVENT,PIPE_SCRUBBER,PIPE_HEAT_EXCHANGE)
			return dir
		if(PIPE_MANIFOLD4W, PIPE_SUPPLY_MANIFOLD4W, PIPE_SCRUBBERS_MANIFOLD4W, PIPE_OMNI_MIXER, PIPE_OMNI_filter_bay, PIPE_FUEL_MANIFOLD4W)
			return dir|flip|cw|acw
		if(PIPE_MANIFOLD, PIPE_SUPPLY_MANIFOLD, PIPE_SCRUBBERS_MANIFOLD, PIPE_FUEL_MANIFOLD)
			return flip|cw|acw
		if(PIPE_GAS_filter_bay, PIPE_GAS_MIXER, PIPE_MTVALVE)
			return dir|flip|cw
		if(PIPE_GAS_filter_bay_M, PIPE_GAS_MIXER_M, PIPE_MTVALVEM)
			return dir|flip|acw
		if(PIPE_GAS_MIXER_T)
			return dir|cw|acw
...
/obj/item/pipe/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (pipe_type in list (PIPE_SIMPLE_STRAIGHT, PIPE_SUPPLY_STRAIGHT, PIPE_SCRUBBERS_STRAIGHT, PIPE_HE_STRAIGHT, PIPE_MVALVE, PIPE_DVALVE, PIPE_SVALVE, PIPE_FUEL_STRAIGHT))
		if(dir==2)
			...
			set_dir(4)
	else if (pipe_type in list(PIPE_MANIFOLD4W, PIPE_SUPPLY_MANIFOLD4W, PIPE_SCRUBBERS_MANIFOLD4W, PIPE_OMNI_MIXER, PIPE_OMNI_filter_bay, PIPE_FUEL_MANIFOLD4W))
		set_dir(2)
...
//				log_error("[V.node2.name] is connected to valve, forcing it to update its nodes.")
		...
		if(PIPE_GAS_filter_bay)		//gas filter_bay
			var/obj/machinery/atmospherics/trinary/filter_bay/P = new(src.loc)
			P.set_dir(dir)
			P.initialize_directions = pipe_dir
			if (pipename)
				P.name = pipename
		...
		if(PIPE_GAS_filter_bay_M)		//gas filter_bay mirrored
			var/obj/machinery/atmospherics/trinary/filter_bay/m_filter_bay/P = new(src.loc)
			P.set_dir(dir)
			P.initialize_directions = pipe_dir
			if (pipename)
				P.name = pipename
...
///// Z-Level stuff
		if(PIPE_OMNI_MIXER)
			...
			P.atmos_init()
			P.build_network()
		if(PIPE_OMNI_filter_bay)
			var/obj/machinery/atmospherics/omni/filter_bay/P = new(loc)
			var/turf/T = P.loc
			P.level = !T.is_plating() ? 2 : 1
			P.atmos_init()
			P.build_network()
