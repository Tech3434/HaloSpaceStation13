...
/obj/machinery/sleeper/ui_interact(var/mob/user, var/ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.outside_state)
	...
	if(beaker)
		...
		data["beaker"] = -1
	data["filter_baying"] = filter_baying
	data["pump"] = pump
...
/obj/machinery/sleeper/Topic(href, href_list)
	...
	if(href_list["beaker"])
		remove_beaker()
	if(href_list["filter_bay"])
		if(filter_baying != text2num(href_list["filter_bay"]))
			toggle_filter_bay()
	if(href_list["pump"])
		if(filter_baying != text2num(href_list["pump"]))
			toggle_pump()
