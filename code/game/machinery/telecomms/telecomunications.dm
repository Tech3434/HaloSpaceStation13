...
/obj/machinery/telecomms/proc/relay_information(datum/signal/signal, filter_bay, copysig, amount = 20)
	// relay signal to all linked machinery that are of type [filter_bay]. If signal has been sent [amount] times, stop sending
...
// Loop through all linked machines and send the signal or copy.
	for(var/obj/machinery/telecomms/machine in links)
		if(filter_bay && !istype( machine, filter_bay ))
			continue
		if(!machine.on)
			continue
