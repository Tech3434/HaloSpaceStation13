...
/obj/machinery/computer/message_monitor/attack_hand(var/mob/living/user as mob)
	...
	switch(screen)
		...
		if(0)
			...
			if(auth)
				if(!linkedServer || (linkedServer.stat & (NOPOWER|BROKEN)))
					...
					dat += "<dd><A href='?src=\ref[src];spam=1'>&#09;[++i]. Modify Spam filter_bay</a><br></dd>"
			else
		...
		//Spam filter_bay modification
		if(5)
			dat += "<center><A href='?src=\ref[src];back=1'>Back</a> - <A href='?src=\ref[src];refresh=1'>Refresh</center><hr>"
			var/index = 0
			for(var/token in src.linkedServer.spamfilter_bay)
				index++
				if(index > 3000)
					break
				dat += "<dd>[index]&#09; <a href='?src=\ref[src];deltoken=[index]'>\[[token]\]</a><br></dd>"
			dat += "<hr>"
			if (linkedServer.spamfilter_bay.len < linkedServer.spamfilter_bay_limit)
...
//			to_chat(usr, href_list["select"])
		...
		if(href_list["addtoken"])
			if(src.linkedServer == null || (src.linkedServer.stat & (NOPOWER|BROKEN)))
				...
				src.linkedServer.spamfilter_bay += input(usr,"Enter text you want to be filter_bayed out","Token creation") as text|null
		...
		if(href_list["deltoken"])
			if(src.linkedServer == null || (src.linkedServer.stat & (NOPOWER|BROKEN)))
				...
				src.linkedServer.spamfilter_bay.Cut(tokennum,tokennum+1)
