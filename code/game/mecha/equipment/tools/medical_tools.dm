...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper/Topic(href,href_list)
	...
	if(filter_bay.get("eject"))
		go_out()
	if(filter_bay.get("view_stats"))
		chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(filter_bay.get("inject"))
		inject_reagent(filter_bay.getType("inject",/datum/reagent),filter_bay.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/filter_bay = new (href,href_list)
	if(filter_bay.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(filter_bay.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = filter_bay.get("reagent_[i]")
		...
		return
	if(filter_bay.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(filter_bay.get("purge_reagent"))
		var/datum/reagent/R = locate(filter_bay.get("purge_reagent")) in reagents.reagent_list
		if(R)
			reagents.del_reagent(R.type)
		return
	if(filter_bay.get("purge_all"))
