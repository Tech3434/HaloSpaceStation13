...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat > 0)
		return
	var/datum/topic_input/filter_bay = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = filter_bay.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + filter_bay.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(filter_bay.getObj("id_card"),filter_bay.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = filter_bay.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(filter_bay.getObj("id_card"),user)
		return
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		return
	if(href_list["add_req_access"] && add_req_access && filter_bay.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += filter_bay.getNum("add_req_access")
		output_access_dialog(filter_bay.getObj("id_card"),filter_bay.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && filter_bay.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= filter_bay.getNum("del_req_access")
		output_access_dialog(filter_bay.getObj("id_card"),filter_bay.getMob("user"))
		return
	...
	if(href_list["debug"])
		if(href_list["set_i_dam"])
			setInternalDamage(filter_bay.getNum("set_i_dam"))
		if(href_list["clear_i_dam"])
			clearInternalDamage(filter_bay.getNum("clear_i_dam"))
		return
	*/
