...
/atom
	...
	var/simulated = 1 //filter_bay for actions - used by lighting overlays
...
/*
 *	atom/proc/search_contents_for(path,list/filter_bay_path=null)
 * Recursevly searches all atom contens (including contents contents and so on).
 *
 * ARGS: path - search atom contents for atoms of this type
 *	   list/filter_bay_path - if set, contents of atoms not of types in this list are excluded from search.
 *
...
/atom/proc/search_contents_for(path,list/filter_bay_path=null)
	...
	for(var/atom/A in src)
		...
		if(filter_bay_path)
			var/pass = 0
			for(var/type in filter_bay_path)
				pass |= istype(A, type)
			if(!pass)
				continue
		if(A.contents.len)
			found += A.search_contents_for(path,filter_bay_path)
	return found
